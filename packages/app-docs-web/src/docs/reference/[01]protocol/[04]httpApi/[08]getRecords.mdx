...

{
  "records": [...],
  "linked_records": [...],
  "next_page": "?page_size=30&page_start=2023-12-15T11:20:01.2240000Z"
}
```

### Filter by type and author

Querying all records is rarely useful, instead we may want add a filter to only get records of a specific type. Here we add a filter for [Post records](/todo) created by the user with entity `cyril.baq.run`.
