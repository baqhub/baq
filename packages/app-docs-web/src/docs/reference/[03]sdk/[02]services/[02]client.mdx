...
- **<props.code>ofUrl(entityRecordUrl, signal?)</props.code>**: <props.pill>Client</props.pill>
...
- **<props.code>authenticated(state: <props.pill href="831f781d#authenticationstate">AuthenticationState</props.pill>)</props.code>**: <props.pill>Client</props.pill>
  ...
  - Such object is obtained through [app registration](831f781d) and [authorization flow](/todo).
...
- **<props.code>getEntityRecord()</props.code>**: <props.code>Promise\<<props.pill href="375c4ea9">EntityRecord</props.pill>\></props.code>
...
- **<props.code>deleteRecord(linkModel, record, opts?)</props.code>**
  ...
  - Delete a record by updating it with a [record tombstone](/todo).
...
const authStateParsed = JSON.parse(authStateJson);
const authState = AuthenticationState.decode(authStateParsed);
...
Once we have a client, we can use it to fetch a record by author entity and ID. Here we fetch a [Post record](/todo) and we ignore linked records altogether.
...
async function createPostRecord(client, text) {
  ...
  const {record} = client.postRecord(
    IO.unknown,
    PostRecord,
    postRecord
  );
...
async function addAvatar(client, entityRecord, avatarBlob) {
  const blobResponse = client.uploadBlob(avatarBlob);
