...
A common use-case for a query filter is to restrict the records by author and type. In this example, we want all of the [File records](/todo) created by user Alice.
...
Here we want all [files](/todo) and [folders](/todo) in a given parent folder. We filter by author and type, but also require the link at `content.parent` to either be empty (root folder) or to point to the provided folder record.
