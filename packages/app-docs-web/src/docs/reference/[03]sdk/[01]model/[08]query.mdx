...
The same query object used to build HTTP requests can also be used to filter a local collection of Records. Here we want to find the most recent [Message record](/todo) for each conversation.
