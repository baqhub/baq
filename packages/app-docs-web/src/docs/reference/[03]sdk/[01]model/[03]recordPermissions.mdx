...
For common use-cases, presets are provided on the `RecordPermissions` type. Here we create a public [Post record](/todo) visible to all.
...
For more advanced situations, permissions can be crafted by hand to fit the need. Here we create a [Task record](/todo) with read access for the author and more two people, but we only notify one of them.
