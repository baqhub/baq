...
On top of simple CRUD operations for records, the protocol offers convenient higher-level features. All of which are implemented on top of records.
...
This record is used to define identity of an app and the permissions it requires to operate. It also identifies the credentials to use when authenticating incoming requests from that app.

This makes it easy possible for users to list all the apps currently in use on their account.
...
When a new record is created, the server looks at the content of the `permissions` object to identify who else is allowed to read it and transfer the record to their respective servers.
...
Due to the potential large number of subscriptions, records propagated in that way should not be expected to be delivered in real-time.
