...
```cli
npm install @baqhub/sdk-react @baqhub/sdk
```
...
We can now add it to the. To do so, we need to provide it with the authentication state provided by the `useAuthentication` hook:
...
It provides an `updateRecords()` function that synchronously reflects the changes instantly across the rest of the UI. The store then works in the background to synchronize the updated data with the server.
...
The same hook is used to update:
